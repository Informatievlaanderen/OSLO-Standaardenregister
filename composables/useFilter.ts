...
const sanitizeCheckboxFilter = (filter: FilterOption, activeFilter: any) => {
  ...
  return allowedVals.length > 0 && filter.key !== ALL ? { [filter.key]: allowedVals } : {};
}
...
export const sanitizeFilters = (filters: FilterOption[], activeFilters: Array<string[]>): SanitizedFilter => {
  ...
  activeFilters.forEach((activeFilter, i) => {
    ...
    if (filter.type === FilterType.CHECKBOX) {
      sanitizedFilter = { ...sanitizedFilter, ...sanitizeCheckboxFilter(filter, activeFilter) };
    } else if (activeFilter) {
      sanitizedFilter = { ...sanitizedFilter, ...sanitizeOtherFilter(filter, activeFilter) };
    }
  })
